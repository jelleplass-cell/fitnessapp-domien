...
interface ExerciseCategoriesListProps {
...
export function ExerciseCategoriesList({ initialCategories }: ExerciseCategoriesListProps) {
  ...
  const handleBulkDelete = async () => {
    const res = await fetch("/api/exercise-categories/bulk-delete", {
      method: "POST",
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCategories([...categories, { ...newCategory, exerciseCount: 0 }]);
        setIsAddOpen(false);
        resetForm();
        router.refresh();
      } else {
  ...
  const handleEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/exercise-categories/${selectedCategory.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
      ...
      if (res.ok) {
        ...
        setCategories(
          categories.map((c) =>
            c.id === selectedCategory.id ? { ...updated, exerciseCount: c.exerciseCount } : c
          )
        );
        setIsEditOpen(false);
        resetForm();
  ...
  const openEdit = (category: ExerciseCategory) => {
  ...
  const openDelete = (category: ExerciseCategory) => {
    setSelectedCategory(category);
    setError(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isAddOpen} onOpenChange={(open) => { setIsAddOpen(open); if (!open) resetForm(); }}>
        <DialogTrigger asChild>
          <Button>
            ...
            Nieuwe oefeningencategorie
          </Button>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Nieuwe oefeningencategorie</DialogTitle>
            <DialogDescription>
              Maak een nieuwe categorie aan om oefeningen te organiseren.
            </DialogDescription>
          </DialogHeader>
      ...
      {filteredCategories.length === 0 && hasFilters ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="p-8 text-center">
            <p className="text-gray-500">Geen oefeningencategorie&euml;n gevonden met deze filters</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="py-12 text-center text-gray-500">
            <p>Nog geen oefeningencategorie&euml;n aangemaakt</p>
            <p className="text-sm mt-1">Klik op &quot;Nieuwe oefeningencategorie&quot; om te beginnen</p>
          </div>
        </div>
            ...
            <div key={category.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
              ...
              <div className="px-6 pb-6">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span>{category.exerciseCount} oefeningen</span>
                </div>
              </div>
            </div>
          ))}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-400 whitespace-nowrap flex-shrink-0">
                  {category.exerciseCount} oefeningen
                </span>
      ...
      <Dialog open={isEditOpen} onOpenChange={(open) => { setIsEditOpen(open); if (!open) resetForm(); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Oefeningencategorie bewerken</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Oefeningencategorie verwijderen</DialogTitle>
            <DialogDescription>
              Weet je zeker dat je de categorie &quot;{selectedCategory?.name}&quot; wilt verwijderen?
              {selectedCategory && selectedCategory.exerciseCount > 0 && (
                <span className="block mt-2 text-yellow-600">
                  Let op: Er zijn nog {selectedCategory.exerciseCount} oefeningen gekoppeld aan deze categorie.
                  Deze oefeningen worden niet verwijderd, maar verliezen hun categorie.
                </span>
              )}
            </DialogDescription>
          </DialogHeader>
      ...
      <BulkActionBar
        ...
        onCancel={bulk.clear}
        entityName="oefeningencategorie&euml;n"
      />
    </div>
  );
}
